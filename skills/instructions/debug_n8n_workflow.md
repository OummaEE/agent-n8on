...
## 6. Уровни уверенности перед ремонтом

| Уровень | Значение | Действие |
|---------|----------|----------|
| CONFIRMED | Причина ясна из execution data | Чинить |
| LIKELY | Паттерн ошибки совпадает, но причина предполагается | Чинить, сообщить допущение |
| UNCONFIRMED | Ошибка неоднозначна | Спросить пользователя |
| BAILOUT | Ремонт невозможен без ручного действия | Прекратить, объяснить |

## 7. Когда НЕ пытаться чинить (bailout)

- Credential отсутствует в n8n — агент не может создавать credentials
- OAuth токен просрочен — нужна авторизация через браузер
- API требует платный план — агент не может подписаться
- Race condition / timing — повтор не поможет стабильно
- Нод требует ручной настройки в UI
- Файл слишком большой для лимитов n8n
